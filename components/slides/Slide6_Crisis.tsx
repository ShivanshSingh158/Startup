...
const RatioIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 text-orange-400" viewBox="0 0 20 20" fill="currentColor">
        <path d="M8 4a1 1 0 100 2h4a1 1 0 100-2H8zM2 8a1 1 0 011-1h14a1 1 0 110 2H3a1 1 0 01-1-1zm0 5a1 1 0 011-1h14a1 1 0 110 2H3a1 1 0 01-1-1z" />
    </svg>
);
...
const Slide6_Crisis: React.FC<SlideProps> = ({ isActive }) => {
  const [visible, setVisible] = useState(false);
  const ratioCounter = useAnimatedCounter(235, 1500, visible);
  const nonFunctionalCounter = useAnimatedCounter(40, 1500, visible);
  const concentrationCounter = useAnimatedCounter(60, 1500, visible);
  ...
  return (
    <div className="w-full h-full flex flex-col justify-center p-12">
      <h2 className={`text-4xl font-bold text-teal-300 mb-8 transition-opacity duration-700 ${visible ? 'opacity-100' : 'opacity-0'}`}>The EV Infrastructure Crisis</h2>
      <div className="grid grid-cols-2 gap-12 flex-grow">
        <div className="flex flex-col justify-center gap-8">
            ...
            <div className={`flex items-center gap-4 transition-all duration-500 ease-out delay-200 ${visible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}`}>
                <RatioIcon/>
                <div>
                    <p className="text-4xl font-bold text-slate-100">1:<span className="text-teal-400">{ratioCounter}</span></p>
                    <p className="text-lg text-slate-400">Charger-to-EV Ratio</p>
                </div>
            </div>
            {/* Statistic 2 */}
            <div className={`flex items-center gap-4 transition-all duration-500 ease-out delay-400 ${visible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}`}>
                <BrokenChargerIcon/>
                <div>
                    <p className="text-4xl font-bold text-slate-100"><span className="text-teal-400">{nonFunctionalCounter}%</span></p>
                    <p className="text-lg text-slate-400">of Stations Non-Functional</p>
                </div>
            </div>
            {/* Statistic 3 */}
            <div className={`flex items-center gap-4 transition-all duration-500 ease-out delay-600 ${visible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}`}>
                <RatioIcon/>
                <div>
                    <p className="text-4xl font-bold text-slate-100"><span className="text-teal-400">{concentrationCounter}%</span></p>
                    <p className="text-lg text-slate-400">of Chargers in 5 Metros</p>
                </div>
            </div>
        </div>
